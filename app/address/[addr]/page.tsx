  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        const sumRes = await fetch(`/api/node/address/${addr}`, { cache: 'no-store' })
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="qtx-card p-5">
        <div className="flex items-start gap-4">
          ...
          <div className="w-12 h-12 rounded-lg flex items-center justify-center shrink-0"
            ...
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#06b6d4" strokeWidth="1.5">
              <circle cx="12" cy="8" r="4"/><path d="M4 20c0-4 3.6-7 8-7s8 3 8 7"/>
            </svg>
          </div>
          <div className="flex-1 min-w-0">
            <div className="text-xs text-qtx-dim uppercase tracking-wide mb-1 font-medium">Address</div>
            <div className="flex flex-wrap items-center gap-2">
              <code className="font-mono text-sm text-qtx-cyan break-all">{addr}</code>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'QTX Balance', value: formatQTX(balance), color: 'text-qtx-cyan', icon: '◈' },
          { label: 'Total Received', value: formatQTX(received), color: 'text-qtx-green', icon: '↙' },
          { label: 'Total Sent', value: formatQTX(sent), color: 'text-red-400', icon: '↗' },
          { label: 'Transactions', value: txCount.toLocaleString(), color: 'text-qtx-purple-light', icon: '⇄' },
        ].map(({ label, value, color, icon }) => (
          <div key={label} className="stat-card">
            <div className="text-xs text-qtx-dim uppercase tracking-wide mb-2 font-medium">{icon} {label}</div>
        ...
        <div className="overflow-x-auto">
          <table className="qtx-table">
            ...
            <tbody>
              {txs.length === 0 ? (
                <tr>
                  <td colSpan={7} className="text-center py-12 text-qtx-dim text-sm">
                ...
                <tr key={`${tx.id}-${i}`}>
                  ...
                  <td className="hidden sm:table-cell">
                    <Link href={`/block/${tx.blockHeight}`} className="text-xs text-qtx-muted hover:text-qtx-cyan font-mono">
                      #{tx.blockHeight}
                    </Link>
                  </td>
                  ...
                  <td>
                    {tx.sender === addr ? (
                      <span className="font-mono text-xs bg-qtx-surface2 text-qtx-dim px-2 py-0.5 rounded">Self</span>
                    ) : (
                  ...
                  <td>
                    <span className={`font-mono text-xs font-medium ${tx.receiver === addr ? 'text-qtx-green' : 'text-red-400'}`}>
                      {tx.receiver === addr ? '+' : '-'}{formatQTX(tx.amount)}
                    </span>
                  </td>
                  <td>
