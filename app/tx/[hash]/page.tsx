    ...
    async function load() {
      try {
        ...
        for (let i = count - 1; i >= 0; i--) {
          const res = await fetch(`/api/node/block/${i}`, {
            cache: "no-store",
          });
          if (!res.ok) continue;
          const block = await res.json();
          const found = (block.body?.txs_list || []).find(
            (t: any) => t.id === txHash,
          );
          if (found) {
            setTx(found);
  ...
  if (error || !tx || !blockInfo)
    return (
      <div className="text-center py-20">
        ...
        <p className="text-qtx-muted mb-2 text-sm">
          Could not find transaction:
        </p>
        <p className="font-mono text-xs text-qtx-cyan break-all max-w-lg mx-auto mb-6">
          {txHash}
        </p>
        <Link
          href="/txs"
          className="px-4 py-2 border border-qtx-cyan text-qtx-cyan rounded text-sm hover:bg-qtx-cyan/10 transition-colors"
        >
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-2 text-xs text-qtx-dim">
        ...
        <span>›</span>
        <Link href="/txs" className="hover:text-qtx-cyan">
          Transactions
        </Link>
        <span>›</span>
        <span className="font-mono text-qtx-muted">
          {shortHash(txHash, 12, 8)}
        </span>
      </div>
      ...
      <div className="qtx-card">
        <Row label="Transaction Hash">
          <span className="font-mono text-sm text-qtx-cyan break-all">
            {txHash}
          </span>
        </Row>
        <Row label="Status">
        ...
        <Row label="Block">
          <Link
            href={`/block/${h.height}`}
            className="text-qtx-cyan hover:underline font-mono"
          >
            #{h.height}
          </Link>
          <span className="text-qtx-dim text-xs ml-3">
            {h.timestamp && h.timestamp > 1_000_000 ? timeAgo(h.timestamp) : ""}
          </span>
        </Row>
        <Row label="Timestamp">
          <span>
        ...
        <Row label="Value">
          <span className="font-mono text-qtx-green font-medium">
            {formatQTX(tx.amount)}
          </span>
          <span className="text-qtx-dim text-xs ml-2">
            ({tx.amount?.toString()} nQTX)
          </span>
        </Row>
        <Row label="Transaction Fee">
          <span className="font-mono text-qtx-muted">
            {tx.gas_limit && tx.gas_price
              ? formatFee(tx.gas_limit, tx.gas_price)
              : "—"}
          </span>
        </Row>
        <Row label="Gas Limit">
          <span className="font-mono text-qtx-muted">
            {tx.gas_limit !== undefined
              ? Number(tx.gas_limit).toLocaleString()
              : "—"}
          </span>
        </Row>
        <Row label="Gas Price">
      ...
      <div className="qtx-card">
        <div className="px-5 py-3 border-b border-qtx-border bg-qtx-surface">
          <h2 className="text-sm font-semibold text-qtx-text">
            SPHINCS+ Signature
          </h2>
          <p className="text-xs text-qtx-dim mt-0.5">
